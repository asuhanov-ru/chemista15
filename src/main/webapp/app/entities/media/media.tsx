  ...
  return (
    <div>
      <h2 id="media-heading" data-cy="MediaHeading">
        <Translate contentKey="chemista15App.media.home.title">Media</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="chemista15App.media.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/media/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="chemista15App.media.home.createLabel">Create new Media</Translate>
          </Link>
        </div>
      </h2>
        ...
        {mediaList && mediaList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="chemista15App.media.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('fileDesc')}>
                  <Translate contentKey="chemista15App.media.fileDesc">File Desc</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('fileDesc')} />
                </th>
                <th>
                  <Translate contentKey="chemista15App.media.collection">Collection</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {mediaList.map((media, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{media.collection ? <Link to={`/collection/${media.collection.id}`}>{media.collection.name}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/media/${media.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/media/${media.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="chemista15App.media.home.notFound">No Media found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={mediaList && mediaList.length > 0 ? '' : 'd-none'}>
